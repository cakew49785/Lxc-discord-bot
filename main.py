...
DISCORD_TOKEN = "MTQ2MDY5MTQxNDQ1NDM3NDQ2MQ.G28xSY.AEVYj_O1o_Epx9-KK1W-zmE9cIWnYPT_hZoUDc" 
MAIN_ADMIN_ID = 1217747285463531522        # Your Discord User ID
VPS_USER_ROLE_ID =    1462117527776788702 # Role ID for VPS owners (or 0 to auto-create)
DEFAULT_STORAGE_POOL = "default"           # Your LXC storage pool name
BOT_LOGO_URL = "https://cdn.discordapp.com/avatars/1460691414454374461/a_df9b9e4fa4165457a36fbdb916d127a4.gif?size=1024" 
# ----------------------------------
...
# --- DATABASE ENGINE (ENHANCED & LOCK-PROOF) ---

# Global lock to prevent Python threads from writing simultaneously
db_thread_lock = threading.Lock()

def get_db():
    """
    Creates a database connection with high timeout and WAL mode enabled.
    """
    # timeout=60.0 makes SQLite wait 60 seconds before throwing "database locked"
    # check_same_thread=False allows connection sharing (handled via db_thread_lock)
    conn = sqlite3.connect('vps.db', timeout=60.0, check_same_thread=False)
    conn.row_factory = sqlite3.Row
    
    # Enable WAL mode for concurrency (Readers don't block Writers)
    try:
        conn.execute('PRAGMA journal_mode=WAL;')
        conn.execute('PRAGMA synchronous=NORMAL;') # Faster writes, slightly less safe on power loss
...
def get_vps_data() -> Dict[str, List[Dict[str, Any]]]:
    ...
    try:
        ...
        logger.error(f"Error reading VPS data: {e}")
        return {}
    finally:
        conn.close()
...
def save_vps_data():
    """
    Saves VPS data to DB. 
    Uses the global lock to ensure no other thread writes at the same time.
    """
    with db_thread_lock:
        conn = get_db()
        try:
            ...
            logger.error(f"Database Locked during save: {e}")
            # Optional: Add retry logic here if needed, but Timeout=60s usually fixes it
        except Exception as e:
...
def resource_monitor():
    ...
    while resource_monitor_active:
        try:
            ...
            # Only log occasionally or if high to reduce I/O
            if cpu_usage > 50 or ram_usage > 50:
                logger.info(f"Monitor: CPU {cpu_usage:.1f}%, RAM {ram_usage:.1f}%")
            ...
            if cpu_usage > CPU_THRESHOLD or ram_usage > RAM_THRESHOLD:
                ...
                try:
                    ...
                    # Acquire lock before modifying VPS data
                    with db_thread_lock:
...
async def on_ready():
    ...
    # Check storage pool
    try:
        await execute_lxc(f"lxc storage show {DEFAULT_STORAGE_POOL}")
        logger.info(f"Storage pool '{DEFAULT_STORAGE_POOL}' found.")
    except Exception:
...
async def set_status(ctx, activity_type: str, *, name: str):
    ...
    if activity_type.lower() not in types:
        ...
        return
    # Restart the loop to avoid overriding
...
if __name__ == "__main__":
    if DISCORD_TOKEN and DISCORD_TOKEN != "YOUR_BOT_TOKEN_HERE":
        ...
        logger.error("No valid Discord token found. Please set DISCORD_TOKEN in the script.")
